...
let rec eval (e : expr) (env : value env) : int =
    ...
    | Call(Var f, eArg) -> 
      ...
      | Closure (f, xl, fBody, fDeclEnv) ->
        let xyList = List.zip xl eArg
        let xVals =
          List.fold (fun (acc, curEnv) (x, y) ->
                      let z = Int(eval y curEnv);
                      (((x, z)::acc), ((x, z)::curEnv))) 
                       ([], env) xyList |> fun (l', env') -> l'
        //let xVal = Int(eval eArg env)
        let fBodyEnv = List.rev xVals @ (f, fClosure) :: fDeclEnv
        eval fBody fBodyEnv
...
let ex1 = Letfun("f1", ["x"; "y"], Prim("+", Var "x", Var "y"), 
                 Call(Var "f1", [CstI 12; CstI 2]));;
